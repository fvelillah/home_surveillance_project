...
2. Enter the Sub-Stream URL (e.g., Channel 2):
   ```
   http://admin:your_password@192.168.1.100/cgi-bin/mjpg/video.cgi?channel=2&subtype=1
   ```
...
curl -I -u admin:your_password "http://192.168.1.100/cgi-bin/mjpg/video.cgi?channel=2&subtype=1"

# 2. Decode Sub-Stream with ffprobe (<0.5s response)
ffprobe "http://admin:your_password@192.168.1.100/cgi-bin/mjpg/video.cgi?channel=2&subtype=1"

# 3. Decode Main-Stream (3K) with ffprobe
ffprobe "http://admin:your_password@192.168.1.100/cgi-bin/mjpg/video.cgi?channel=2&subtype=0"

# 4. Fetch a single JPEG snapshot
curl -u admin:your_password "http://192.168.1.100/cgi-bin/snapshot.cgi?channel=2" -o /tmp/cam2_test.jpg
```
...
DAHUA_NVR_IP=192.168.1.100
DAHUA_NVR_HTTP_PORT=80
DAHUA_NVR_USER=admin
DAHUA_NVR_PASSWORD=your_password
DAHUA_NUM_CAMERAS=9
...
- **7.1 USB & Sub-Stream Video Discovery (`scripts/embed_and_index_baseline.py`)**: Direct in-memory extraction pipeline from mounted external USB storage (`/mnt/e/NVR/<backup_date>/` or `E:\NVR\<backup_date>\`) supporting Dahua NVR naming schema (`NVR_ch<ch>_extra1_<starttime>_<endtime>.asf`) without copying video files to local hard drive.
- **7.2 Direct In-RAM Embedding & MiniBatchKMeans Clustering (`scripts/embed_and_index_baseline.py`)**: Stream-decodes 10s Sub-Stream windows directly into RAM (16 frames @ 224x224), generates PyTorch MobileNetV3-Small / EfficientNet-B0 L2-normalized embeddings, fits in-memory `MiniBatchKMeans` (500 centroids per channel across all 9 cameras), and batch upserts to Central Qdrant Cloud (`POST /api/v1/baseline/index` -> `COLLECTION_NAME="anomaly_baseline"`) and local Qdrant Edge shard (`data/qdrant_edge/`).
- **7.3 Evaluation & Threshold Calibration (`scripts/evaluate_thresholds.py`)**: Computes empirical kNN cosine distance distribution (95th/99th percentiles) on normal footage to tune `EDGE_TRIAGE_THRESHOLD` (~0.060) and `CLOUD_ANOMALY_THRESHOLD` (~0.150) in `.env`.
...
### Overall Progress: `31 / 31 Tasks Completed (100.0%)`
...
[x] Phase 6: Multi-Camera Web Console (6/6)
[x] Phase 7: Calibration & Deployment (5/5)
```
...
### Phase 6: Multi-Camera Web Console & Live UI
- [x] **Task 6.1**: Setup Next.js 14 project shell, dark mode theme, and global state (`SurveillanceContext.tsx`, `globals.css`).
- [x] **Task 6.2**: Implement 3x3 responsive live camera grid (`CameraGrid.tsx`, `CameraPanel.tsx`) with direct HTTP stream proxies and per-channel anomaly badges.
- [x] **Task 6.3**: Implement real-time alert queue (`AlertQueue.tsx`, `ActiveIncidentCard.tsx`) with severity gauges and audible notifications.
- [x] **Task 6.4**: Implement synchronized incident video modal (`IncidentModal.tsx`) with baseline nearest-neighbor comparison grid.
- [x] **Task 6.5**: Implement 24-hour continuous multi-camera timeline (`ContinuousTimeline.tsx`, `AnomalyMeter.tsx`).
- [x] **Task 6.6**: Implement semantic search console and interactive AI security copilot drawer (`OpsCopilot.tsx`).

### Phase 7: Baseline Calibration, Evaluation & Production Deployment
- [x] **Task 7.1**: Implement USB Sub-Stream video discovery and Dahua ASF filename parser (`NVR_ch<ch>_extra1_<start>_<end>.asf`) for direct in-memory ingestion (`scripts/embed_and_index_baseline.py`).
- [x] **Task 7.2**: Implement in-RAM PyTorch feature extraction, MiniBatchKMeans clustering (500 centroids/camera), and direct Qdrant Cloud / Edge batch upsert (`scripts/embed_and_index_baseline.py`).
- [x] **Task 7.3**: Implement threshold calibration and ROC evaluation suite (`scripts/evaluate_thresholds.py`).
- [x] **Task 7.4**: Build production Docker Compose configs (`docker-compose.prod.yml`), systemd service units, and automated log rotations.
- [x] **Task 7.5**: Execute end-to-end integration and penetration test across all 9 camera feeds (`tests/test_baseline_indexer_script.py`, `tests/test_threshold_evaluation_script.py`, `tests/test_e2e_phase7_integration.py`).
...
# 6. Phase 7: Baseline Indexer, Threshold Evaluation & End-to-End Production Tests
uv run pytest tests/test_baseline_indexer_script.py tests/test_threshold_evaluation_script.py tests/test_e2e_phase7_integration.py

# 7. Full Workspace Test Suite (all 147+ unit & integration tests)
uv run pytest
```
...
# 4. Direct USB Baseline Extraction & Qdrant Cloud Ingestion (Phase 7.2)
uv run python scripts/embed_and_index_baseline.py --video-dir /mnt/e/NVR/2026-9-19 --clusters-per-camera 500
uv run python scripts/embed_and_index_baseline.py --video-dir /mnt/e/NVR/2026-9-19 --dry-run
```
