...
async def handle_escalation(request: EscalationRequest) -> EscalationResult:
    ...
    1. Re-embeds video with Twelve Labs Marengo or local PyTorch Model Server.
    2. Scores embedding against the central Qdrant baseline.
    3. Fuses cloud and edge scores via EnsembleScorer with temporal boosting.
    4. Records metrics and returns structured escalation verdict.
    """
    ...
    if request.clip_data:
        ...
        if twelvelabs_client.is_enabled():
            try:
                ...
                upload_result = twelvelabs_client.upload_video(tmp_path, index_type="marengo")
    ...
    # Step 3: Query Central Qdrant Baseline
    ...
    incident_id = f"inc-{escalation_id}" if is_confirmed else None

    # Step 5: Update registries and telemetry
