...
@app.websocket("/api/v1/ws/stream/{channel}")
async def stream_live_websocket(websocket: WebSocket, channel: int, fps: Optional[int] = 12):
    """
    High-performance WebSocket video stream endpoint.
    Transmits JPEG frames directly over a single full-duplex socket per camera,
    completely bypassing browser HTTP/1.1 6-connection limits across 9 simultaneous camera feeds.
    """
    await websocket.accept()
    target_fps = fps or 12
    frame_interval = 1.0 / max(target_fps, 1)
    ...
    try:
        while True:
            ...
            if app_state and channel in app_state.capture_manager.workers:
                frame = app_state.capture_manager.get_latest_frame(channel)
                if frame is not None:
        ...
        logger.debug("WebSocket client closed on channel %d: %s", channel, exc)
